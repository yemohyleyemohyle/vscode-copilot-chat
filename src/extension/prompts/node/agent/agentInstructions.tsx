	...
	async render(state: void, sizing: PromptSizing) {
		...
		return <InstructionMessage>
			...
			<Tag name='toolUseInstructions'>
				...
				{hasMultiReadFileTool && <>Prefer the {ToolName.ReadFile} tool for single file reads and the {ToolName.MultiReadFile} tool when you need to read multiple files (2+ files) in a single operation, or when you need to read multiple chunks from a large file. This is significantly more efficient than calling {ToolName.ReadFile} multiple times and should be your first choice for: analyzing multiple configuration files, comparing implementations across files, understanding project structure, reading multiple sections from a large log file, or any scenario where you need multiple read operations at once.<br /></>}
