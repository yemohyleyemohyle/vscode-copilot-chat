	...
	for (const message of messages) {
		...
		for (let chunkIndex = 0; chunkIndex < chunks.length; chunkIndex++) {
			...
			if (message.role === 'assistant' && logService) {
				logService.info(`[model.message.added] direction=${messageDirection} messageUuid=${messageUuid} headerRequestId=${headerRequestId} chunk=${chunkIndex + 1}/${chunks.length} messageJson=${chunks[chunkIndex]}`);
			}
		}
	}
